// ============================================================
// lib/dropbox.js — Dropbox access for the portal.
//
...
//
// Reads are the common case. The ONE write path is writeFile(), used only
// so a user can edit their OWN User Framework (Layer 2) profile files —
// never firm rules or agent files. writeFile() needs the Dropbox app to
// hold the files.content.write scope; if it does not, Dropbox returns a
// missing-scope error which we surface as err.missingScope so the UI can
// explain. Dependency-free (uses the built-in global fetch).
// ============================================================
...
async function readFile(path) {
  ...
  if (!r.ok) {
    ...
    // Attach structured detail so callers can tell "this file isn't there"
    // (normal) from "Dropbox is broken/disconnected" (not normal). Message text
    // is unchanged, so existing callers behave exactly as before.
    const err = new Error('dropbox download error: ' + (t || r.status));
    err.status = r.status;
    err.notFound = /path\/not_found/.test(t);
    throw err;
  }
  return await r.text();
}

// Overwrite a single file's UTF-8 text content. This is the ONLY write path in
// the portal and is used strictly for a user editing their own User Framework
// (Layer 2) profile files. Parent folders are created automatically by Dropbox.
// Requires the app's files.content.write scope — without it Dropbox returns a
// missing-scope error, surfaced here as err.missingScope so a caller/UI can say
// "ask an admin to enable write access" instead of a generic failure.
async function writeFile(path, content) {
  ...
  const r = await fetch('https://content.dropboxapi.com/2/files/upload', {
    ...
    headers: {
      ...
      'Dropbox-API-Arg': apiArg({ path: path, mode: 'overwrite', autorename: false, mute: true }),
      'Content-Type': 'application/octet-stream',
    },
    body: Buffer.from(String(content == null ? '' : content), 'utf8'),
  });
  if (!r.ok) {
    const t = await r.text().catch(() => '');
    const err = new Error('dropbox upload error: ' + (t || r.status));
    err.status = r.status;
    err.missingScope = /missing_scope|insufficient_scope|no_write_permission/i.test(t);
    throw err;
  }
...
module.exports = {
  ...
  getAccessToken, listFiles, readFile, writeFile, CALLBACK_URL,
};
