...
(async () => {
  ...
  // ---- say/do gap: memory-claim without a real save ----
  await test('memory claim without a real save is flagged', async () => {
    const r = await validateOutput("Got it — I'll remember to address you as Tzipora.", { channel: 'chat', memorySaved: false });
    assert.ok(has(r, 'memory-claim-unsaved'), 'a claimed save with memorySaved=false must be flagged');
  });

  await test('memory claim WITH a real save passes', async () => {
    const r = await validateOutput("Saved. I'll remember to address you as Tzipora.", { channel: 'chat', memorySaved: true });
    assert.ok(!has(r, 'memory-claim-unsaved'), 'a claimed save with memorySaved=true must NOT be flagged');
  });

  await test('memory claim check is skipped when memorySaved is not provided', async () => {
    const r = await validateOutput("I'll remember that.", { channel: 'chat' });
    assert.ok(!has(r, 'memory-claim-unsaved'), 'no memorySaved in ctx -> check skipped (legacy mode)');
  });
