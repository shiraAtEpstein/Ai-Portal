...
async function searchMail(userId, opts) {
  ...
  try {
    ...
    for (const id of ids) {
      ...
      // 2026-09-16 (Shira): now also requesting Cc, and keeping threadId off
      // the raw response — both needed so task-hub.js can tell "I'm only
      // CC'd on this, not a direct recipient" apart from a normal To, and
      // can check whether the thread already moved on since. Nothing here
      // changes existing callers: both fields are additive on the entry.
      if (!includeBody) url += '&metadataHeaders=From&metadataHeaders=To&metadataHeaders=Cc&metadataHeaders=Subject&metadataHeaders=Date';
...
// Is `address` one of the recipients in a raw To/Cc header value? Handles
// "Name <addr>, Name2 <addr2>" and bare "addr, addr2" forms. Used to tell
// whether the signed-in user was actually addressed, or only carbon-copied.
function headerHasAddress(headerValue, address) {
  if (!headerValue || !address) return false;
  const needle = String(address).trim().toLowerCase();
  if (!needle) return false;
  return String(headerValue).toLowerCase().split(',').some(function (part) {
    const m = part.match(/<([^>]+)>/);
...
// but it's not to me, no need to surface it [as a task]." Read-only,
// best-effort lookup of how many messages are now in a thread — task-hub.js
// uses this only for CC-only candidates, to tell a still-quiet CC (nobody's
// replied yet — worth a task) apart from a CC on a thread that already moved
// on without needing the signed-in user at all. Returns null on any failure
// (network hiccup, token issue, deleted thread) so the caller can fail OPEN
// — an unknown thread state must never be the reason a real task disappears.
async function getThreadActivity(userId, threadId) {
...
module.exports = {
  ...
  headerHasAddress, getThreadActivity,
  SCOPES, CALLBACK_URL,
};
