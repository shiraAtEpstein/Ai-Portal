...
// { connected, email, canDraft } for the UI — no token is touched.
async function connectionStatus(userId) {
  const p = getPool();
  if (!p) return { connected: false, email: null, canDraft: false };
  const r = await p.query('SELECT email, scope FROM gmail_connections WHERE user_id = $1', [userId]);
  const row = r.rows[0];
  if (!row) return { connected: false, email: null, canDraft: false };
  return { connected: true, email: row.email || null, canDraft: hasDraftScope(row.scope) };
}
