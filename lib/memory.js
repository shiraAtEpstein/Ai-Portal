...
async function defaultInfer({ userText, assistantText }) {
  ...
  const sys = [
    ...
    '   or anything specific to a case. Forbidden.',
  ...
  const user = "The USER just wrote:\n" + String(userText || '').slice(0, 2000) +
    ...
    'Return JSON of ONLY preferences the USER explicitly expressed in their message, or [] if none.';
  const r = await client.messages.create({ model: MODEL, max_tokens: 300, system: sys, messages: [{ role: 'user', content: user }] });
...
module.exports = {
  loadForUser, observe, remember, forget, listForUser, clearStaged,
