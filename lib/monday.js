...
// The "person in charge" (paralegal / deal_owner) name for the monday deal that
// is linked to this WhatsApp group, or null if there's no linked deal / no
// person set / monday not configured. This is the single responsible staff
// member for the case. Read-only.
async function responsibleNameForGroup(groupJid) {
  if (!isConfigured()) return null;
  try {
    const deal = await resolveDealForGroupId(groupJid);
    if (!deal) return null;
    const prof = PROFILES.find((p) => String(p.id) === String(deal.monday_board_id));
    const col = prof && prof.columns && prof.columns.paralegal;
    if (!col) return null;
    const d = await gql(
      `query($ids:[ID!]){ items(ids:$ids){ id column_values(ids:["${col}"]){ id text } } }`,
      { ids: [String(deal.monday_item_id)] }
    );
...
module.exports = { isConfigured, findUser, myDeals, renderDeals, resolveBoard, boardColumns, boardColumnList, boardItems, renderBoard, renderColumnList, readQuery, findClientByPhone, resolveDealsForClient, resolveDealForGroupId, responsibleNameForGroup };
