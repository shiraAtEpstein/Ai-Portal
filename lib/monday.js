  ...
  while (page && pages < MAX_PAGES) {
    for (const it of (page.items || [])) {
      for (const cv of (it.column_values || [])) {
        ...
        if (!existing) {
          index.set(norm, { items: [{ monday_item_id: id, name: it.name || null }] });
        } else if (!existing.items.some((x) => x.monday_item_id === id)) {
          // Same phone on more than one client row (e.g. spouses / co-buyers
          // sharing a number). Keep ALL of them. The strict singular matcher
          // (findClientByPhone) still treats >1 as ambiguous so message
          // attribution never guesses; the plural matcher (findClientsByPhone)
          // hands the whole set to deal-finding, where co-buyers on the same
          // deal resolve to that deal anyway.
          existing.items.push({ monday_item_id: id, name: it.name || null });
        }
      }
    }
    if (!page.cursor) break;
...
async function findClientByPhone(normalizedPhone) {
  try {
    ...
    // No hit, or a phone shared by MORE THAN ONE client row (ambiguous) —
    // treat as "not confidently matched" so the message-attribution caller
    // leaves it unresolved. Deal-finding uses findClientsByPhone instead.
    if (!hit || !hit.items || hit.items.length !== 1) return null;
    return hit.items[0];
  } catch (_) {
    return null;
  }
...
// Deal-finding variant: return EVERY client row sharing this phone (0..n),
// WITHOUT the singular ambiguity bail. Co-buyers on the same deal all lead to
// that deal, so the caller can union their deals and let the group-name match
// pick the right matter. Safe because it never attributes a message to a
// person — it only helps find which deal a whole group is about.
async function findClientsByPhone(normalizedPhone) {
  try {
    const key = normalizePhone(normalizedPhone);
    if (!key) return [];
    const index = await _getClientsIndex();
    const hit = index.get(key);
    return (hit && hit.items) ? hit.items.slice() : [];
  } catch (_) {
    return [];
  }
}
...
// Forces the NEXT resolveDealForGroupId() to rebuild the index instead of
// serving the up-to-30-minute-old cached one. Added 7 Sept: someone pasting a
// group id into monday and immediately checking the review screen could be
// looking at a Map built before they saved it. Cheap — resets a timestamp,
// doesn't touch monday itself; the rebuild happens lazily on next use.
function invalidateDealGroupIndex() { _dealGroupIndexAt = 0; }
...
module.exports = { isConfigured, findUser, myDeals, renderDeals, resolveBoard, boardColumns, boardColumnList, boardItems, renderBoard, renderColumnList, readQuery, findClientByPhone, findClientsByPhone, resolveDealsForClient, enrichDealsWithContext, resolveDealForGroupId, invalidateDealGroupIndex, responsibleNameForGroup, responsibleNameForDeal };
