...
const CATEGORY = { REQUIRED: 'required', NONE: 'none', POTENTIAL: 'potential' };
...
// items: [{ key, text }]. Returns Map<key, category>. Missing key = not
// classified yet (AI unavailable/failed/omitted) — caller retries later.
async function evaluateNeedsReply(items, { model } = {}) {
  const result = new Map();
  const list = (Array.isArray(items) ? items : []).filter((it) => it && it.key != null);
  if (!list.length) return result;

  // Layer 1 — deterministic net: obvious closers -> 'none' immediately.
  const ambiguous = [];
  for (const it of list) {
    if (isObviousCloser(it.text)) result.set(it.key, CATEGORY.NONE);
    else ambiguous.push(it);
  }
  console.log(`[unanswered/needs-reply] ${list.length} msg(s): ${list.length - ambiguous.length} -> none (obvious closer), ${ambiguous.length} -> AI`);
