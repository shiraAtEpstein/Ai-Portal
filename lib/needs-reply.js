...
// A block whose only readable content is a voice/media placeholder — no typed
// words. These get their OWN status ('voice'): surfaced on the board, but not
// alerted as "required" (we can't read them yet). A voice note alongside real
// typed text is NOT voice-only — the text is classified normally.
const MEDIA_PLACEHOLDER_RE = /\[(voice message|audio|image|video|document[^\]]*|sticker|location|contact card)\]/gi;
function isVoiceOnly(text) {
  const raw = String(text == null ? '' : text);
  if (!/\[(voice message|audio)\]/i.test(raw)) return false;      // must contain a voice note
  const rest = raw.replace(MEDIA_PLACEHOLDER_RE, ' ').replace(/\s+/g, ' ').trim();
  return rest.length === 0;                                        // nothing but voice/media
}
...
async function evaluateNeedsReply(items, { model } = {}) {
  ...
  for (const it of list) {
    if (isVoiceOnly(it.text)) { result.set(it.key, CATEGORY.VOICE); voiceCount++; }
