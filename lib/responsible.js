...
// Fallback when monday gives no usable match at all: the most recently
// active staff member in THIS chat (config/staff-directory.json is the
// source for which phone9 is which staffer). Skips the partner (inAllGroups)
// UNLESS he is the only staffer who has ever messaged here — a group with no
// other signal shouldn't be left unassigned. Returns a staff record or null
// (nobody on staff has ever sent a message in this chat).
...
async function resolveAndStore(jid, dir, { write = true } = {}) {
  ...
  try {
    ...
    if (lastSender && !lastSender.inAllGroups && (!mondayMatch || lastSender.email !== mondayMatch.email)) {
      ...
      // No usable monday match at all -> whoever's actually been
      // corresponding, falling back to the partner only if he's the sole
      // staffer who has ever messaged here.
      match = await lastActiveStaffFallback(jid, dir);
