...
// The most recently active NON-PARTNER staff member in this chat — used for
// the "does the live correspondent override monday" check.
//
// --- 2026-09-15 (Shira, revised again) --------------------------------------
// Earlier version: if the single most-recent sender was the partner (Yaacov
// Epstein), that meant flat "no override" — monday's assignment stood, full
// stop. Shira's correction: that's not right either. If the last active
// person was Yaacov, DON'T just give up — look further back for the most
// recent staffer who ISN'T him, and let THAT person override monday. Only
// when NO non-partner staffer has EVER been active in this chat (Yaacov is
// truly the only one who's ever spoken) does it fall through to leaving
// monday's assignment alone — nothing is ever assigned TO Yaacov himself.
// Must also be a real current participant of the group (same ground-truth
// rule as everywhere else in this file — a name with no membership doesn't
// count). Returns a staff record or null.
async function mostRecentNonPartnerActiveStaff(jid, dir, group) {
  const staff = (dir && dir.staff) || [];
  let actives;
  try {
    actives = await ingestDb.lastActiveStaffPhone9s(jid);
  ...
  for (const a of actives) {
    const s = byPhone.get(String(a.phone9));
    if (!s || s.inAllGroups) continue;           // skip the partner, keep looking further back
    if (!staffIsParticipant(s, group)) continue; // must actually be in the group
    return s;                                    // first (= most recent) non-partner participant
  }
  return null;
}
...
async function resolveAndStore(jid, dir, { write = true } = {}) {
  ...
  try {
    ...
    // whoever's most recently active in the chat overrides monday's
    // assignment and gets it "for now". EXCEPTION: nothing is ever assigned
    // TO the partner (Yaacov Epstein) — if he's the most recent sender, look
    // further back for whoever was active before him (see
    // mostRecentNonPartnerActiveStaff above); only when nobody else has EVER
    // been active here does monday's assignment (or the no-monday fallback
    // below) stand.
    const lastSender = await mostRecentNonPartnerActiveStaff(jid, dir, g);
    if (lastSender && (!mondayMatch || lastSender.email !== mondayMatch.email)) {
      match = lastSender;
...
module.exports = {
  ...
  mostRecentNonPartnerActiveStaff,
};
