...
async function resolveAndStore(jid, dir) {
  ...
  try {
    ...
    if (match && !staffIsParticipant(match, g)) {
      console.log(`[responsible] "${jid}" monday match "${match.email}" is not a participant in the group — discarding`);
      match = null;
    }

    // No usable monday match (none linked, no name match, or discarded above)
    // -> whoever has actually been corresponding here.
    if (!match) {
