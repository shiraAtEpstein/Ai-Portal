...
// Upload input data to the Files API so the sandbox can READ it as a file,
// instead of the model retyping the whole dataset into code (which overflows
// the inner max_tokens on large reports). Returns the uploaded file_id.
async function uploadInputFile(name, mime, buf) {
  ...
  if (!r.ok) throw new Error('file upload ' + r.status + ' ' + (await r.text()).slice(0, 200));
  const j = await r.json();
  return j.id;
}

// Build a document from an instruction + data. Returns { files: [{id, filename, mime, url}] }.
// For anything beyond a little data, the data is uploaded as a file into the
// container and the model is told to READ it — so the model's own output stays
// small and never truncates, no matter how big the report is.
async function buildDocument({ userId, instruction, data, format }) {
  ...
  const useFile = dataStr.length > 1500;

  let inputFileId = null;
  const parts = [];
  if (useFile) {
    inputFileId = await uploadInputFile('data.txt', 'text/plain', Buffer.from(dataStr, 'utf8'));
    parts.push({ type: 'container_upload', file_id: inputFileId });
    console.log('[BUILD_DOC] uploaded input data as file ' + inputFileId + ' (' + dataStr.length + ' chars) for the sandbox to read');
  }
  const text = String(instruction || 'Create a document.') +
    (useFile
      ? '\n\nThe FULL dataset for this file has been uploaded into your container as a text file (run `ls` in the working directory to find it, e.g. "data.txt", then read it). Do NOT expect the data inline in this message and do NOT retype it — read it from the file.'
      : (dataStr ? '\n\nUse this data:\n' + dataStr : '')) +
  ...
  // Collect output file_ids, excluding the input data file we uploaded.
  const ids = Array.from(new Set(collectFileIds(resp.content))).filter((id) => id !== inputFileId);
  const metas = [];
  for (const id of ids) {
