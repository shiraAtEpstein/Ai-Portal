...
function start() {
  ...
  // 2026-09-17: the "refresh every active user's Task Hub on a timer" job
  // used to be started HERE as well as in lib/task-hub.js's
  // startServerSideSweep() — server.js called both unconditionally at boot
  // (unansweredScheduler.start() and task-hub's startServerSideSweep()),
  // so two independent 5-minute loops were each calling the exact same
  // taskHub.refreshTasks(userId) for every active user, unaware of each
  // other. isNewCandidate()'s dedup meant a *settled* item was never
  // double-triaged, but the two loops could and did overlap on a BRAND NEW
  // candidate — both checking isNewCandidate() before either loop's insert
  // had committed — which is a real way to double-spend a Claude call, not
  // just double the Gmail/monday/WhatsApp-DB read traffic. There was also
  // no timeout protection here, unlike task-hub.js's sweepAllUsers() (see
  // its withTimeout() wrapper, added 2026-09-15 after a hung Gmail call
  // silently wedged that loop for every user behind the stuck one).
  //
  // Removed. lib/task-hub.js's startServerSideSweep() (started once from
  // server.js, already timeout-protected) is now the ONLY place this runs.
  // If TASK_HUB_AUTO_REFRESH_SECONDS is set anywhere (Render env), it no
  // longer does anything — use TASK_HUB_SWEEP_INTERVAL_SECONDS instead to
  // change this cadence going forward.
}
