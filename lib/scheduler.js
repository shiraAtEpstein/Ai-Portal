...
function start() {
  ...
  // anything to click in — it should do it automatically." Runs the exact
  ...
  // clicking. This is what makes a voice note actually turn into a task
  ...
  //
  // COST NOTE, flagged rather than assumed away: refreshTasks() already
  // skips re-spending a Claude call on anything already triaged
  // (isNewCandidate() in lib/task-hub.js), so the Claude cost per tick
  // scales with what's actually NEW, not with how many users/tasks exist.
  // But every tick still calls out to Gmail / monday.com / the WhatsApp DB
  // for every active user, whether or not anything changed — that traffic
  // used to happen only when a human opened Task Hub and clicked Refresh
  // (rate-limited to once/2min per person), and now happens on a timer for
  // everyone regardless of activity. Default interval kept at 5 minutes —
  // same cadence as the classifier backlog-drain above — to keep that
  // baseline load reasonable; lower TASK_HUB_AUTO_REFRESH_SECONDS (min 60)
  // for something snappier, e.g. while testing.
