...
// reached yet stays flagged — the same fail-safe the digest uses. A chat cleared
// by hand on the board stops counting from the moment it was cleared.
//
...
async function fetchOwned(pool, days) {
  const r = await pool.query(`
    ...
    SELECT lower(trim(COALESCE(ovr.email, wg.responsible_email)))          AS owner_email,
           (c.opened_at AT TIME ZONE '${TZ}')::date                        AS opened_date,
           ((COALESCE(c.replied_at, dis.dismissed_at)) AT TIME ZONE '${TZ}')::date AS closed_date,
           (c.replied_at IS NULL AND dis.dismissed_at IS NULL AND c.needs_reply) AS still_open,
           EXTRACT(EPOCH FROM (now() - c.opened_at))                       AS waiting_s
    FROM conv c
  ...
  return r.rows.map((row) => ({
    ...
    openedDate: isoDate(row.opened_date),
    closedDate: isoDate(row.closed_date),      // null while still open
    stillOpen: !!row.still_open,
    waiting: num(row.waiting_s),
  }));
}
...
function profileFrom(answered) {
  ...
  const bestHour = usable.length ? usable.reduce((a, h) => (h.medianSeconds < a.medianSeconds ? h : a)) : null;
  const bestBand = bands.length ? bands.reduce((a, b) => (b.medianSeconds < a.medianSeconds ? b : a)) : null;
...
async function buildStaffMetrics({
  ...
  for (const email of everyone) {
    ...
    const openOnes = owned.filter((c) => c.stillOpen);
    // THE merge: answered latencies (mine because I replied) + the current wait
    // of everything still sitting with me (mine because nobody replied).
    const latencies = answered.map((a) => a.latency).concat(openOnes.map((c) => c.waiting));
    byEmail.set(email, Object.assign(
      speedFrom(latencies, openOnes.length),
      profileFrom(answered),
      { answeredCount: answered.length, consistency: consistencyFor(owned, spine) }
    ));
  }

  // --- firm-wide ------------------------------------------------------------
  const allAnswered = [...answeredByEmail.values()].flat().concat(firmLineAnswered);
  const allOpen = [...ownedByEmail.values()].flat().filter((c) => c.stillOpen);
  const allOwned = [...ownedByEmail.values()].flat();
  const firm = Object.assign(
    speedFrom(allAnswered.map((a) => a.latency).concat(allOpen.map((c) => c.waiting)), allOpen.length),
    profileFrom(allAnswered),
    { answeredCount: allAnswered.length, consistency: consistencyFor(allOwned, spine) }
  );

  const firmLine = includeFirmLine && firmLineAnswered.length
    ? Object.assign(speedFrom(firmLineAnswered.map((a) => a.latency), 0),
        profileFrom(firmLineAnswered), { answeredCount: firmLineAnswered.length, consistency: null })
    : null;
...
module.exports = {
  buildStaffMetrics,
  // exported for tests
  median, share, consistencyFor, profileFrom, speedFrom, bandOf, localDateSpine,
