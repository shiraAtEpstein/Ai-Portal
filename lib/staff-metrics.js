...
const { NON_MESSAGE_KINDS } = require('../whatsapp/ingest/db');
// Rendered as a SQL list from our own constant — no user input reaches it.
const NON_MESSAGE_SQL = NON_MESSAGE_KINDS.map((k) => "'" + k.replace(/'/g, "''") + "'").join(', ');
...
const STREAKS = `
  WITH base AS (
    ...
    WHERE source = 'whatsapp' AND COALESCE(sent_at, created_at) IS NOT NULL
      -- Same exclusion as the board (whatsapp/ingest/db.js): a reaction is not
      -- a client asking for something, so it must not open a waiting streak
      -- here either — otherwise the medians and the board would disagree about
      -- which conversations were ever waiting. NULL still counts.
      AND (msg_kind IS NULL OR msg_kind NOT IN (${NON_MESSAGE_SQL}))
  ),
  grp AS (
    SELECT *,
