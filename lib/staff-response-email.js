...
// Natural-Hebrew wait label — same wording as the board and the digest.
...
// Percent against the firm median, written in words. A bare "-27%" next to
...
function vsFirmText(pct) {
  if (pct == null) return null;
  if (pct <= -3) return { text: 'מהיר ב-' + Math.abs(pct) + '% מהמשרד', color: '#1f7a44' };
  if (pct >= 3) return { text: 'איטי ב-' + pct + '% מהמשרד', color: '#b0281c' };
  return { text: 'כמו חציון המשרד', color: '#6b7280' };
}
...
// One line per staff member who currently has something waiting. Sorted by how
// long the oldest one has waited, so the top line is the one that matters.
function staffLines(staff) {
  const rows = (staff || [])
    .filter((s) => (s.items || []).length)
    .map((s) => {
      const items = s.items || [];
      const oldest = items.reduce((m, it) => Math.max(m, Number(it.hoursWaiting) || 0), 0);
      return { name: s.name, count: items.length, oldest, vs: vsFirmText(s.replies ? s.vsFirmPct : null) };
    })
  ...
  return rows.map((r) => {
    ...
    return '<tr>' +
      ...
      '<td style="padding:10px 8px;border-bottom:1px solid #f0f0f2;text-align:left;vertical-align:top;font-size:14px;color:#6e8b7c;white-space:nowrap;">' + esc(waitLabel(r.oldest)) + '</td>' +
      '</tr>';
  }).join('');
}
...
function renderStaffReportHtml({ data, recipientName, boardUrl } = {}) {
  ...
  return '' +
    ...
    '<tr><td style="padding:6px 24px 2px 24px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" dir="rtl"><tr>' +
      tile(f.median || '—', 'חציון מענה של המשרד', '#2f3b34', '#eef2ef', '#dbe4dd') +
      tile(openTotal, 'שיחות פתוחות עכשיו', '#6b7280', '#f3f4f6', '#e3e5e9') +
      tile(f.required || 0, '🔴 דורשות מענה', '#b0281c', '#fdeeeb', '#f6d5cd') +
    '</tr></table></td></tr>' +
    '<tr><td style="padding:2px 28px 10px 28px;font-size:11.5px;color:#9aa0a8;text-align:right;">' +
      'חציון המשרד מחושב על כל המענים ב-' + (d.windowDays || 30) + ' הימים האחרונים. אין יעד זמן — כל איש צוות נמדד ביחס לחציון הזה.' +
    '</td></tr>' +
    ...
    '<tr><td style="padding:8px 22px 4px 22px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" dir="rtl">' +
      '<tr>' +
        ...
        '<th style="text-align:left;font-size:12px;color:#8a9099;font-weight:bold;padding:0 8px 8px 8px;border-bottom:1px solid #e6e6ea;">הוותיקה ביותר</th>' +
