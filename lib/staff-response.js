...
function toPendingItem(it) {
  return {
    ...
    // hoursWaiting = WORKING hours, and it feeds the medians only.
    // waitedLabel / elapsedHours = REAL elapsed time, and they are the only
    // thing shown against a row. See lib/wait-label.js.
    hoursWaiting: it.hoursWaiting,
    waitedSince: it.waitedSince || null,
    waitedLabel: it.waitedLabel || '',
    elapsedHours: it.elapsedHours != null ? it.elapsedHours : null,
    waitedTone: it.waitedTone || 'ok',
...
async function buildStaffResponse({ windowDays = 30, trendWeeks = 8, consistencyDays = 14 } = {}) {
  ...
  // Oldest first, by when the client actually wrote. Sorting on working hours
  // ties the whole overnight batch at zero and buries the oldest message.
  const byOldestFirst = (a, b) =>
    new Date(a.waitedSince || 0).getTime() - new Date(b.waitedSince || 0).getTime();
  for (const load of openByEmail.values()) load.items.sort(byOldestFirst);
  unassigned.items.sort(byOldestFirst);
  ...
  const rowFor = (m, load) => {
    ...
    // NOTE the `>= 0`. Under the working clock a chat that arrived after 22:00
    // or on Saturday has waited exactly ZERO working hours so far — it is still
    // very much open, and dropping it here (as a `> 0` filter did) removed it
    // from `measured`, from `openCount`, from the median and from
    // `oldestOpenSeconds`, while `load.open` still counted it. The card then
    // read "5 ממתינות · כולל 0 שממתינות", and at night the oldest-waiting tile
    // went blank with five chats on the list.
    const openWaits = ((load && load.items) || [])
      .map((it) => Number(it.hoursWaiting) * 3600)
    ...
    //
    // And it is measured in REAL elapsed time, not working hours, because it
    // describes ONE message rather than the firm's speed: the honest answer to
    // "how long has the worst one been sitting there" is eleven days, not the
    // seven working days those eleven contained.
    // `it.elapsedHours != null` first: Number(null) is 0, not NaN, so a row with
    // no timestamp would pass isFinite, count as zero hours, and — if it won —
    // leave oldestOpenSeconds at 0 with a null label, printing "—" in full ink
    // instead of the dimmed empty state.
    const openItems = ((load && load.items) || [])
      .filter((it) => it.elapsedHours != null && Number.isFinite(Number(it.elapsedHours)));
    const oldestItem = openItems.length
      ? openItems.reduce((a, b) => (Number(b.elapsedHours) > Number(a.elapsedHours) ? b : a))
      : null;
    const oldestOpenSeconds = oldestItem ? Number(oldestItem.elapsedHours) * 3600 : null;
    // The tile prints this, not a number it formats itself — the wording of a
    // wait lives in exactly one place (lib/wait-label.js).
    const oldestOpenLabel = oldestItem ? (oldestItem.waitedLabel || null) : null;
    const oldestOpenTone = oldestItem ? (oldestItem.waitedTone || 'ok') : null;
  ...
  // Same `>= 0` as in rowFor: a chat that arrived out of hours has zero working
  // hours behind it and must still be counted as open.
  ...
  const firm = {
    ...
    // The clock every number above was measured on. Sent to the page rather
    // than written into it, so the footnote can never drift from the policy in
    // config/business-hours.json.
