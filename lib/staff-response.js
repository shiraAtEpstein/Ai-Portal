...
async function buildStaffResponse({ windowDays = 30, trendWeeks = 8, consistencyDays = 14 } = {}) {
  ...
  const blankMetrics = {
    measured: 0, openCount: 0, answeredCount: 0, medianSeconds: null,
  ...
  const rowFor = (m, load) => {
    ...
    return {
      ...
      // `measured` = answered conversations PLUS the ones still waiting. It is
      // deliberately not called "replies" any more: half of what it counts was
      // never replied to, and that is the point.
      measured: met.measured,
      openCount: met.openCount,
  ...
  const staffRows = measuredStaff.map((s) => {
    ...
    return Object.assign({ name: s.name, email: s.email }, rowFor(metrics.byEmail.get(key), openByEmail.get(key)));
  });
  ...
  const firm = {
    ...
    measured: fm ? fm.measured : 0,
    openCount: fm ? fm.openCount : 0,
    trend: fm ? fm.trend : [],
    hours: fm ? fm.hours : [],
    bands: fm ? fm.bands : [],
