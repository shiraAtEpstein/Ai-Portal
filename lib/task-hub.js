...
async function resolveUserIdByEmail(email) {
  var r = await pool().query('SELECT id FROM users WHERE lower(email) = lower($1)', [email]);
  return r.rows[0] && r.rows[0].id;
}

// Real-time counterpart to fetchManualCandidates() above (2026-09-14). That
// function only ever runs on a page load or the refresh button (2-min
// server-side cooldown) — a message in a linked WhatsApp task-inbox group
// sat invisible until someone happened to reload. This is called
// fire-and-forget from whatsapp/groups/provider.js's _ingest() the moment a
// message lands in a group with task_owner_email already set, so the task
// is extracted and pushed (see lib/task-events.js) right away instead of
// waiting for a pull. Must never throw back into message ingestion.
//
// Safe to run alongside the existing pull path: unified_tasks is UNIQUE
// (user_id, source, source_ref), so if refreshTasks() later re-discovers
// this same message, upsertTask()'s ON CONFLICT just updates the same row
// — never a duplicate, and fetchManualCandidates()'s "since last manual
// task" filter will already skip it (its first_seen_at matches this one).
async function processRealtimeManualMessage(ownerEmail, message) {
  if (!ownerEmail || !message || !message.text) return null;
  try {
    await ensureTables();
    var userId = await resolveUserIdByEmail(ownerEmail);
    if (!userId) return null; // no LAWLY account for this email (yet) — nothing to push to
    var extracted = await runSkill('manual-task-extract', message.text);
    if (extracted.skip) return null;
    var firstSeenAt = message.eff_at || new Date().toISOString();
    await upsertTask(userId, 'manual', message.source_item_id, extracted, firstSeenAt, null);
    var res = await pool().query(
      "SELECT * FROM unified_tasks WHERE user_id = $1 AND source = 'manual' AND source_ref = $2",
      [userId, message.source_item_id]
    );
    var task = res.rows[0];
    if (task) {
      var eff = effectivePriority(task);
      task = Object.assign({}, task, {
        effective_priority: eff,
        escalated: !task.priority_overridden_by_user && eff !== task.priority
      });
      taskEvents.publish(userId, task);
    }
    return task || null;
  } catch (e) {
    console.warn('[task-hub] real-time manual-task extraction failed for', ownerEmail, e.message);
    return null;
  }
}
...
module.exports = {
  refreshTasks, listTasks, listAllTasks, addManualTask, patchTask, toggleTask,
  processRealtimeManualMessage
};
