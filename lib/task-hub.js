...
function ensureTables() {
  ...
  _tablesReady = p.query(
    ...
    // 2026-09-10 (Shira): "open in Gmail / WhatsApp" button on the task
    // cards needs a real URL, not just source_ref — self-provisioned like
    // every other column here, no manual migration step.
...
async function fetchEmailCandidates(userId) {
  ...
  return (result.messages || []).map(function (m) {
    return {
      ...
      // Gmail's own "deep link into a specific message" trick — works
      // against the message id searchMail already gives us, no extra call.
...
// A URL to open the chat itself, so the task card's "open WhatsApp" button
// actually goes somewhere:
//   - 1:1 chat: same wa.me/972<last-9-digits> convention lib/unanswered-
//     digest.js's waLink() already uses for the client-response digest.
//   - group chat: wa.me is 1:1-only (WhatsApp exposes no URL that opens a
//     chat by JID) — the only thing that opens a group directly is its
//     invite link. `whatsapp_groups.invite_link` was backfilled for most
//     live groups by an earlier project (see
//     claude/whatsapp-group-invite-links.md); this is a best-effort read of
//     that column and returns null — never throws — if the column or a
//     match isn't there.
...
async function refreshTasks(userId) {
  ...
  // "monday" here is deals pulled as CONTEXT (for the UI's pulled-counts
  // display), not tasks created — monday no longer produces its own tasks.
  // email/whatsapp/manual are still the TOTAL candidates currently pulled
  // from that source (unchanged meaning, kept for anyone already reading
  // this response) — `triaged` is new: how many of those actually went to
  // Claude this refresh, after skipping ones already triaged before.
  return {
...
// ---------------------------------------------------------------------------
// Admin overview — every open task, across every user, in one query. Used by
// the "Team Tasks" admin view (public/admin-tasks.html). Same shape as
// listTasks() (effective_priority/escalated computed the same way) plus
// user_id/user_name/user_email on every row so the page can group by person.
// Callers must gate this on requireAdmin themselves — this function does not
// check roles, same division of responsibility as everything else in
// lib/task-hub.js (routes/*.js owns auth, lib/*.js owns data).
// ---------------------------------------------------------------------------
...
// Admin edit — same field whitelist as patchTask (title/estimated_minutes/
// priority only, never status/done_at), but scoped by id alone instead of
// `WHERE user_id = $1`, since the caller here is an admin editing a task
// that belongs to someone else (from the "Team view" in mytasks.html).
// Callers must gate this on requireAdmin themselves (routes/mytasks.js's
// PATCH /admin/:id) — same division of responsibility as listAllTasks().
// There is deliberately no admin equivalent of toggleTask(): an admin can
// edit or reprioritize any task here, but can never mark someone else's
// task done — that stays limited to POST /:id/toggle, which is still
// scoped to req.session.userId only.
...
module.exports = { refreshTasks, refreshManualTasks, listTasks, listAllTasks, addManualTask, patchTask, patchTaskAsAdmin, toggleTask };
