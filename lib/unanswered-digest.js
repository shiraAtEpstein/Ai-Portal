...
async function buildDigest({ hours = 3 } = {}) {
  ...
  const verdicts = await evaluateNeedsReply(
    chats.map((c) => ({ key: c.chat_jid, text: c.blockText || c.lastText, lastMsgAgeMinutes: ageMinutes(c.lastInboundAt) }))
  );
...
async function buildBoard() {
  ...
  for (const chat of chats) {
    const cat = verdicts.get(chat.chat_jid); // required | potential | none | voice | undefined(pending)
    if (cat === 'none') continue;            // no response needed -> not on the board
    const status = cat === 'required' ? 'required'
      : cat === 'potential' ? 'potential'
