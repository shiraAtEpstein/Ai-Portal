...
async function buildDigest({ hours = 3 } = {}) {
  ...
  for (const chat of flagged) {
    ...
    all.push(item);
    // The responsible is the monday "person in charge" for this group's case,
    // resolved ONCE and cached on the group. Route the alert to that single
    // person only. Fallback: the default owner (no linked deal / no name match).
    ...
    const person = email ? (dir.staff || []).find((s) => s.email === email) : null;
    const target = person || owner;
    console.log(`[unanswered]   route "${chat.label}" -> ${target.email}${person ? ' (monday responsible)' : ' (default owner — no monday responsible)'}`);
    if (!byPerson[target.email]) byPerson[target.email] = { name: target.name, items: [] };
    byPerson[target.email].items.push(item);
  }
