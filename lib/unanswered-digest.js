...
// Where the "פתח ב-Lawly" button points. For now the portal home; once the
// dedicated "הודעות שממתינות" page exists, point PUBLIC_BASE_URL + that path.
...
function renderHtml({ recipientName, items, isFullList, hours }) {
  ...
  return '' +
    ...
    '<tr><td style="background:#2b2f38;padding:22px 28px;text-align:right;">' +
      '<div style="color:#9aa1ac;font-size:12px;letter-spacing:.3px;margin-bottom:6px;">משרד עפשטיין · תזכורת יומית</div>' +
    ...
    '<tr><td style="padding:18px 28px 4px 28px;">' +
      '<a href="' + esc(PORTAL_URL) + '" style="display:inline-block;background:#2b2f38;color:#ffffff;text-decoration:none;font-size:15px;font-weight:bold;padding:13px 30px;border-radius:6px;">פתיחת רשימת השיחות ←</a>' +
    '</td></tr>' +
    ...
    // footer
    '<tr><td style="padding:14px 28px 24px 28px;font-size:11.5px;color:#aab0b8;text-align:right;line-height:1.7;border-top:1px solid #eee;">נשלח אוטומטית על ידי Lawly · משרד עפשטיין. השיחה יורדת מהרשימה ברגע שמישהו במשרד עונה בוואטסאפ. הזמנים מבוססים על מועד קליטת ההודעה; אם החיבור היה מנותק ייתכן איחור קל.</td></tr>' +
    '</table></div>';
}
...
module.exports = { buildDigest, sendDigests, renderHtml };
