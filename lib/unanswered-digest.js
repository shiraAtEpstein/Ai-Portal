...
async function buildDigest({ hours = 3 } = {}) {
  ...
  // AI triage: send every candidate's last client message to Claude and keep
  // only those that actually need a reply (drops pure "תודה"/"👍" closers).
  // Fail-safe: anything the AI can't judge stays flagged (see lib/needs-reply).
  const verdicts = await evaluateNeedsReply(
    chats.map((c) => ({ key: c.chat_jid, text: c.lastText }))
  );
