...
// slug -> { at, value }  where value is { text, files }
...
// Public: load a user's framework, cached. Returns { text, files } where text is
// '' when the user has no framework (empty-state -> pure Firm Core). Passing
// opts.reader (tests) bypasses the cache.
async function loadForEmail(email, opts = {}) {
  const slug = slugForEmail(email);
  if (!slug) return { text: '', files: [] };
  const now = Date.now();
  if (!opts.reader) {
