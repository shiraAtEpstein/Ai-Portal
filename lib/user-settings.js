...
// Should this admin be emailed for a system alert (e.g. the WhatsApp groups
// connector needs a human to rescan a QR code)? Same opt-out shape as
// ruleProposal, reusing the existing systemAlerts type rather than adding
// a new one.
function wantsSystemAlertEmail(stored) {
  const s = withDefaults(stored);
  return !!(s.notifications.channels.email && s.notifications.types.systemAlerts);
}

module.exports = {
  defaults, sanitize, withDefaults, wantsRuleProposalEmail, wantsSystemAlertEmail,
