...
// 2026-09-16 (Shira): "why does a voice note need to wait for a poll —
// shouldn't it just trigger?" Cheap, single-column lookup so
// whatsapp/groups/provider.js's _ingest() (the live per-message hot path)
// can check "is this chat someone's personal task-inbox group" without
// pulling the whole row — used only for messages that are already known to
// be an audioMessage, so it costs nothing on the vastly more common case of
// a text message. null when the group isn't linked (not an error — most
// groups aren't anyone's task inbox) or has been removed.
async function getTaskOwnerEmail(providerGroupJid) {
  await ensureTables();
  const p = getPool();
  if (!p || !providerGroupJid) return null;
  const r = await p.query(
    `SELECT task_owner_email FROM whatsapp_groups
     WHERE provider_group_jid = $1 AND task_owner_email IS NOT NULL AND removed_at IS NULL
     LIMIT 1`,
    [providerGroupJid]
  );
  return (r.rows[0] && r.rows[0].task_owner_email) || null;
}
