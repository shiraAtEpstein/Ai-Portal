...
async function enqueueJob({
  ...
  // 2026-09-10 (Shira): WhatsApp/Baileys can deliver the same message id
  // more than once — e.g. a content-less protocol envelope (classified
  // 'stub'/'noContent') alongside a separate delivery carrying the real
  // text. With plain ON CONFLICT DO NOTHING, whichever one landed FIRST
  // won permanently, and a real message that happened to arrive after its
  // own stub-shaped sibling was lost for good — invisible everywhere that
  // reads processing_jobs (Task Hub, the unanswered board, staff-response
  // dashboard), not logged as an error anywhere, because nothing failed —
  // a row genuinely existed, it just never got the real content.
  // Confirmed live 10 Sep: a real staff message stored as msg_kind='stub'
  // with no sibling row holding the actual text anywhere in the table.
  // Fix: a later delivery is now allowed to UPGRADE a row that's still
  // sitting at a "weak" classification (NULL, or one of NON_MESSAGE_KINDS)
  // — but never overwrites a row that already holds real content, so a
  // genuine duplicate/redelivery of the same real message still just
  // no-ops as before.
  const r = await p.query(
    `INSERT INTO processing_jobs
     ...
     ON CONFLICT (source, source_item_id) DO UPDATE SET
       ...
       chat_jid = COALESCE(processing_jobs.chat_jid, EXCLUDED.chat_jid),
       is_group = processing_jobs.is_group OR EXCLUDED.is_group,
       direction = COALESCE(processing_jobs.direction, EXCLUDED.direction),
       sender_phone = COALESCE(processing_jobs.sender_phone, EXCLUDED.sender_phone),
       sender_staff_phone9 = COALESCE(processing_jobs.sender_staff_phone9, EXCLUDED.sender_staff_phone9),
       contact_id = COALESCE(processing_jobs.contact_id, EXCLUDED.contact_id),
       deal_id = COALESCE(processing_jobs.deal_id, EXCLUDED.deal_id),
       sent_at = COALESCE(processing_jobs.sent_at, EXCLUDED.sent_at)
     WHERE processing_jobs.msg_kind IS NULL OR processing_jobs.msg_kind = ANY($12::text[])
     RETURNING id`,
    [
      source_item_id,
      chat_jid || null,
  ...
  // A no-op update (WHERE didn't match — the existing row already had real
  // content) returns no row even though the message id exists. RETURNING
  // id is best-effort telemetry for callers (e.g. the deal/urgent-trigger
  // hookups below) — falling back to null there is the same as before.
